 ...
 *
 * Read from the request rather than from a `NEXT_PUBLIC_*` origin: those are
 * inlined into the client bundle at build time (see the Dockerfile, which takes
 * no build arguments for exactly this reason), so an image built for one host
 * and served from another would hand out share links pointing at the wrong
 * place. And rather than `window.location.origin`, which
